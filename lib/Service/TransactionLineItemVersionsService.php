...
class TransactionLineItemVersionsService
{
    ...
    public function getPaymentTransactionsLineItemVersionsWithHttpInfo($space, $after = null, $before = null, $expand = null, $limit = null, $order = null, string $contentType = self::contentTypes['getPaymentTransactionsLineItemVersions'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Wallee\Sdk\Model\LineItemVersionListResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Wallee\Sdk\Model\LineItemVersionListResponse' !== 'string') {
                            try {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Wallee\Sdk\Model\LineItemVersionListResponse', []),
            ...
            $returnType = '\Wallee\Sdk\Model\LineItemVersionListResponse';
    ...
    public function getPaymentTransactionsLineItemVersionsSearchWithHttpInfo($space, $expand = null, $limit = null, $offset = null, $order = null, $query = null, string $contentType = self::contentTypes['getPaymentTransactionsLineItemVersionsSearch'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Wallee\Sdk\Model\LineItemVersionSearchResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Wallee\Sdk\Model\LineItemVersionSearchResponse' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Wallee\Sdk\Model\LineItemVersionSearchResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                case 400:
            ...
            $returnType = '\Wallee\Sdk\Model\LineItemVersionSearchResponse';
            if ($returnType === '\SplFileObject') {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
     ...
     *
     * @param  int $space Specifies the ID of the space the operation should be executed in. (required)
     * @param  \Wallee\Sdk\Model\TransactionLineItemVersionCreate $transaction_line_item_version_create (required)
     * @param  string[] $expand (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['postPaymentTransactionsLineItemVersions'] to see the possible values for this operation
     *
     * @throws \Wallee\Sdk\ApiException on non-2xx response or if the response body is not in the expected format
